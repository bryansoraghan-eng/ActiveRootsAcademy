...
const Ic = {
  ...
  clients:  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75"/></svg>,
...
const NAV = [
  { to: '/coaching', label: 'Dashboard', icon: Ic.home, end: true },
  { to: '/coaching/clients', label: 'Clients', icon: Ic.clients },
  { to: '/coaching/training', label: 'Training Plans', icon: Ic.training },
  { to: '/coaching/nutrition', label: 'Nutrition', icon: Ic.nutrition },
  { to: '/coaching/progress', label: 'Progress', icon: Ic.progress },
  { to: '/coaching/checkins', label: 'Check-ins', icon: Ic.checkins },
];
      ...
      <aside className={`ara-sidebar${sidebarOpen ? ' ara-sidebar-open' : ''}`}>
        ...
        <div className="ara-sidebar-dashboard">
          {NAV.map(({ to, label, icon, end }) => (
            <NavLink key={to} to={to} end={end} onClick={closeSidebar} className={({ isActive }) => `ara-nav-link${isActive ? ' active' : ''}`}>
              {icon}{label}
            </NavLink>
          ))}
        </div>
        ...
        <div className="ara-sidebar-footer">
          <div className="ara-sidebar-profile">
            <div className="ara-sidebar-avatar" style={{ backgroundColor: '#C4703F' }}>{initials}</div>
            <div style={{ minWidth: 0 }}>
              <div className="ara-sidebar-profile-name">{user?.name}</div>
              <div className="ara-sidebar-profile-role">Online Coach</div>
            </div>
          </div>
          <button type="button" className="ara-signout-btn" onClick={logout}>{Ic.signout}Sign out</button>
        </div>
      </aside>
      ...
      <main className="ara-main-col">
        {isPreviewMode && (
          <div className="ara-preview-banner">
            <span>Admin Preview — Coach Portal</span>
            <button type="button" onClick={() => { exitPreview(); navigate('/'); }}>Exit Preview</button>
          </div>
        )}
        <Outlet />
      </main>
